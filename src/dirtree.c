...
//
// DESCRIPTION: tells with which status the card responded to the previous
...
int get_status_after_command (RESPONSE* resp) {
  ...
  switch(sw1) {
    case SW1_CORRECT: {
      if (sw2 == 0x00) return(SW_NORMAL_ENDING);
      else {
    ...
    case SW1_SAT_BUSY: {
      if (sw2 == 0x00) return(SW_SAT_BUSY);
      else {
        // What is better: exit or continue???
        send_warning(strcat("get_status_after_command: ", WRN_WRONG_STATUS_RETURNED));
        return(SW_WRONG_STATUS_RETURNED);
...
// Here will stay those functions related to the SIM_CARD structure, such as
...
//
// FUNCTION NAME: craete_EF_header
//
...
int print_tree_node(TREE_NODE_TYPE* node) {
  ...
  // declared during the definition of
...
//
// NOTES:
//   - It is necessary to analyze if it's a better implementation to remove an
//     input paramenter, shifting LIST_NODE_TYPE* to_fill on the return value.
//   - We suppose that the list to_fill is empty when this function is called:
//     a control to this prerequisite is needed.
//
TREE_NODE_LIST* find_selectable_partial(SCARDHANDLE hCard,
  ...
  for (file_to_test = 0; file_to_test < EF_list_size; ++file_to_test) {
    if(reset) {
      printf("\nFILE SELECTION OK.\n");
      // Starting from the first child of the root, 3F00, the selection
      // of "starting point" must be done iteratively, selecting all the DF
...
//              current card directory, its father and its brothers. To achieve
...
TREE_NODE_LIST* extract_sons(TREE_NODE_LIST* selectable_copy,
  ...
  while(!finito) {
    ...
    if (
       same_file_id(aux, MF_copy) || same_file_id(aux, current)    //checking for current DF
       || (father != NULL && same_file_id(aux, father))      //checking for father, provided that father of MF doesn't exist.
    ) {
      ...
      if(brothers != NULL) {// I take into account the fact that a node can be an 'only child'.
        int j;
...
int gsm_build_up_tree(SCARDHANDLE hCard, SCARD_IO_REQUEST* dwActiveProtocol,
  ...
  printf("\nDEBUG: sto analizzando il DF %.2X\n", id);
  printf("\nENTERED find_selectable.\n");
  ...
  //DEBUG CODE
  if(!(father == NULL)) {
    printf("\nDEBUG: PADRE DEL NODO CORRENTE: -- FATHER --\n");
    print_tree_node(father);
  }
  if(!(tree == NULL)) {
    printf("\nDEBUG: NODO CORRENTE: -- TREE --\n");
    print_tree_node(tree);
  }
  printf("\nDEBUG: NODO CORRENTE:\n");
  print_tree_node(tree);
  if(!((*tree).p_brothers == NULL)) {
    printf("\nDEBUG: FRATELLI DEL NODO CORRENTE:\n");
  ...
  printf("\nDEBUG: FIGLI TROVATI DA EXTRACT SONS:\n");
  visualizza_file_id_lista(sons);

  if(!(sons == NULL) && !((*sons).n_list_length == 0)) {

    printf("\nNumero figli: %i.\n",(*sons).n_list_length);
    printf("\nNumero figli secondo la SIM: %i.\n",(*tree).sons_num);

    //Check not executed because the number of direct child directory declared
    ...
    */
    ...
    printf("\nDEBUG: gsm_build_up_tree: lista dei file figli di %.2X%.2X.\n",
           blist_get_element((&(*tree).file_id),0),
           blist_get_element((&(*tree).file_id),1));
    ...
    for(i=0; i<(*sons).n_list_length; i++) {
      //Brothers of a node are all the
    ...
    printf("\nDEBUG: gsm_build_tree: Puntatore primogenito inizializzato.\n");
    printf("\nDEBUG: numero_fratelli = %i", (*brothers).n_list_length);
    ...
    if((*tree).firstborn != NULL) {
      printf("\nDEBUG: gsm_build_up_tree: fratelli del primogenito di %.2X%.2X:\n",
             blist_get_element(&(*tree).file_id,0),
             blist_get_element(&(*tree).file_id,1));
    ...
    if(bro != NULL) {
      printf("\nNUMERO DI FRATELLI DA ANALIZZARE = %i\n",
      ...
      for(i=0; i<(*(*son).p_brothers).n_list_length; i++) {
        if(((*bro).file_type == FTYP_DF)) {
          ...
          // 'tree' becomes new father
          // 'bro' becomes tree
          printf("\nRecursion in every brother that is DF...\n");
